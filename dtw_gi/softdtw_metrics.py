"""
This implementation is based on the one (distributed without license) from
...
class SoftDTWFunction(Function):
    ...
    def backward(ctx, grad_output):
        ...
        g_ = gamma.item()
        E = torch.Tensor(compute_softdtw_backward(D_, R_, g_)).to(dev).type(
            dtype
        )
...
class SoftDTW(torch.nn.Module):
    def __init__(self, gamma=1.0, normalize=False):
        ...
        self.gamma = gamma
        self.func_dtw = SoftDTWFunction.apply
...
class SoftDTWWithMap(torch.nn.Module):
    def __init__(self, map, gamma=1.0, normalize=False):
        super(SoftDTWWithMap, self).__init__()
        self.map = map
        self.normalize = normalize
        self.gamma = gamma
        self.func_dtw = SoftDTWFunction.apply
    ...
    def forward(self, x, y):
        ...
        if len(x.shape) < 3:
            ...
            squeeze = True
        if self.normalize:
            D_xy = self.calc_distance_matrix(x, self.map(y))
            out_xy = self.func_dtw(D_xy, self.gamma)
            D_xx = self.calc_distance_matrix(x, x)
            out_xx = self.func_dtw(D_xx, self.gamma)
            D_yy = self.calc_distance_matrix(self.map(y), self.map(y))
            out_yy = self.func_dtw(D_yy, self.gamma)
            result = out_xy - 1 / 2 * (out_xx + out_yy)  # distance
        else:
            D_xy = self.calc_distance_matrix(x, self.map(y))
